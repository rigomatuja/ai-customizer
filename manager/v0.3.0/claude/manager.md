...
- **Paso 2** — Intent → operation (how you translate needs into catalog changes, with extra collaborative ceremony for agent creation and frontmatter questions for skill creation)
...
## 2.11 — Skill creation — frontmatter questions

When `op = create` AND `type = skill`, after the base questions, ask
two extra dimensions that determine the YAML frontmatter (see 4.3):

1. **Description (when + what)** — ask the user for a single sentence
   describing WHEN the skill should fire, front-loading the trigger
   condition. Example: *"when editing React components under
   `ui/src/client/`, enforce the repo's UI conventions"*. This goes in
   the `description` field of BOTH tool variants and is the ONLY thing
   Opencode uses for semantic matching.

2. **Auto-activation paths (Claude Code only)** — ask: *"Should this
   skill auto-activate when you are editing specific files? If yes,
   which globs?"* If yes → collect the globs and emit `paths:` in the
   Claude frontmatter. The Opencode mirror gets NO `paths` field
   (silently ignored there); tell the user explicitly that Opencode
   will only match semantically via `description`.

If the user says "no" to paths, the Claude skill still works —
it just falls back to semantic matching like Opencode does. That is
a valid choice for skills that serve multiple unrelated contexts.

Do NOT fill `paths` without asking. Do NOT derive them from the
skill's `category` — infer them from the user's stated trigger.
