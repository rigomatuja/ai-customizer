...
12. **Model assignment per tool** — ONE dimension, asked once,
    ...
    - **Claude** — the `model:` field in the subagent frontmatter
      ...
      catalog-side static registry at
      `<catalogPath>/.ai-customizer/models/claude.json` via the
      `GET /api/tools/claude-models` endpoint to know current aliases
      and known versions. Default: **omit `model:` entirely (inherit
      from primary)**. Second choice: a bare alias (`sonnet` etc.) —
      Claude resolves it to the latest version. Only pin a full ID
      when the user explicitly asks for a specific version.
    - **Opencode** — the `model:` field in the agent frontmatter
      takes `provider/model-id` (e.g., `anthropic/claude-sonnet-4-5`
      or `openai/gpt-5.4`). Read the state-side detected registry via
      `GET /api/tools/opencode-models`. If it is empty, tell the user
      to refresh from the UI (Settings → Models → Refresh) or run
      `opencode models` themselves first. Default: **omit `model:`
      entirely (inherit)**. When the user picks one, offer the
      detected list; never accept a string that is not in the
      registry (Opencode would fail at runtime).
