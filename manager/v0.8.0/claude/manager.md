...
10. **Opencode mode** — "In Opencode, should this agent be
    `mode: primary` (user Tab-selects it from the TUI) or
    `mode: subagent` (auto-delegated by whichever primary is active
    when `description` matches)?"
    - Pick `primary` when the user drives the invocation explicitly
      (you wrote this for them to use directly).
    - Pick `subagent` when the agent is a behind-the-scenes helper
      the primary calls without user action.
    - Default the question toward `subagent` unless the user clearly
      wants explicit invocation — `subagent` composes better with
      other agents and does not clutter the Tab selector.
...
**Gentle-ai composition check** — if the agent's procedure could
reference gentle-ai skills or slash commands (i.e., the intent
overlaps with what gentle-ai provides: SDD workflow, PR creation,
issue creation, adversarial review, etc.), run the Phase 1 + Phase 2
protocol from 3.9 NOW (before Show-before-write). Propose concrete
references to enumerated skills/commands in the agent's
`## Procedure` section. Body-reference only — do NOT add gentle-ai
skills to `dependencies.customs` (that field is catalog-internal).
...
If the skill's `## What I do` section could reference gentle-ai
skills (e.g., "run `sdd-verify` after X", "invoke `judgment-day`
before committing"), run the Phase 1 + Phase 2 protocol from 3.9
BEFORE Show-before-write. Propose concrete skill id references in
the body. Add `dependencies.gentleAi = { required: true }` to the
...
Two phases. Phase 1 answers "is gentle-ai installed?" via tag scan.
Phase 2 answers "what does gentle-ai offer me?" via filesystem
enumeration of the tool-side directories. Run Phase 1 on-demand
(when the user mentions gentle-ai or could benefit from it). Run
Phase 2 only when Phase 1 confirmed install AND the user is
creating/improving a custom that could compose with gentle-ai
capabilities.

#### Phase 1 — Install detection (tag scan)

Per 0.6 scope with the user: *"I'm going to scan `CLAUDE.md` and
`AGENTS.md` for gentle-ai markers. OK?"*
...
2. Extract every `<!-- gentle-ai:<tag> -->` where `<tag>` matches
   `[a-zA-Z0-9_-]+`. The union of tags across both masters is the
   set of THEMATIC DOMAINS gentle-ai covers (e.g., `persona`,
   `engram-protocol`, `sdd-orchestrator`, `strict-tdd-mode`).
3. Interpret:
   - **Zero tags** → gentle-ai is NOT installed. Tell the user, and
     REFUSE to add `dependencies.gentleAi.required = true`. Skip
     Phase 2.
   - **Tags present** → gentle-ai is installed. Record the tag list
     in the cumulative model. If the user's intent likely benefits
     from gentle-ai, move to Phase 2.

**A tag is NOT a skill id.** It marks a thematic section of the
master. The actual invocable units live in the tool-side skills
and commands directories — that is Phase 2's job to enumerate.

#### Phase 2 — Capability enumeration (tool-side scan)

Per 0.6 scope with the user: *"I'm going to list gentle-ai's
installed skills and slash commands so we can reference them. OK?"*

For Claude:
- `ls ~/.claude/skills/` → skill IDs. Skip `_shared` and any
  directory whose SKILL.md is absent.
- For each skill, `Read ~/.claude/skills/<id>/SKILL.md` — pick up
  `name` and `description` from the frontmatter.
- `ls ~/.claude/commands/` → slash-command names. Skip `manager.md`
  (that's the customizer's own).
- For each command, `Read ~/.claude/commands/<name>.md` frontmatter
  `description`.
- `ls ~/.claude/agents/` → skip `manager.md`; any other `.md` is a
  subagent installed by gentle-ai or another source.

For Opencode:
- `ls ~/.config/opencode/skills/` → same pattern.
- `ls ~/.config/opencode/agent/` → skip `manager.md`; any other is
  an installed agent body.
- Opencode has NO `commands/` directory.

Build a compact mental map before continuing:
...
Summarize compactly to the user BEFORE using it. Example: *"Detected
17 gentle-ai skills (sdd-explore, sdd-propose, sdd-apply, judgment-day,
...) and 9 Claude slash commands (/sdd-init, /sdd-new, ...). Want me
to reference any of these in what you're building?"*

#### Using the enumeration when authoring

**Creating a SKILL that composes with gentle-ai** — reference gentle-ai
skills from the body text, by exact id. Example body:
...
add `minVersion` if the user has one in mind. Do NOT add entries to
`dependencies.customs` for gentle-ai skills — `customs` is for THIS
catalog's skills/agents only. The runtime dependency is signaled
by the `gentleAi.required` flag; the body text carries the
per-skill references.

**Creating an AGENT that orchestrates gentle-ai** — same body-reference
pattern. Distinguish per tool:
...
**Creating a PATCH that targets a gentle-ai tag section** — this is
the primary patch use case when gentle-ai is installed. Patches
OVERRIDE or EXTEND a thematic block of the master.

1. Pick the tag to target (from Phase 1's list).
2. Find its boundaries in the master `.original`:
   - Start: the `<!-- gentle-ai:<tag> -->` line (inclusive).
   - End: just before the next heading of equal or higher level,
     OR just before the next `<!-- gentle-ai:*** -->` marker,
     whichever comes first.
3. The `before.md` is that full block.
4. The `after.md` is the replacement — your rewrite, addition, or
   deletion of that block.
5. Apply the 3.4 rules (read `.original`, not live master; blocking
   validations).
6. Manifest: `dependencies.gentleAi = { required: true }` — the
   patch only makes sense when that tag exists.

#### Rejection of shortcuts

- NEVER hand-author a gentle-ai skill/command reference that you
  haven't verified via Phase 2 enumeration. One typo and the
  reference is dead.
- NEVER add a gentle-ai reference to a custom when Phase 1 returned
  zero tags. The custom would silently break on any machine without
  gentle-ai.
- NEVER equate a tag with a skill id. `persona` is a tag; it is NOT
  a skill at `~/.claude/skills/persona/`. If the user asks about a
  name, figure out whether they mean a tag or a skill before
  responding.

#### User-facing metadata

If the user asks *"what does gentle-ai expose?"* and you have not
yet run Phase 2, run it now (with scoping). Then present the compact
summary. Also mention: *"The UI's Settings → Gentle AI integration
shows the same tag scan — the enumeration itself is manager-only."*
...
```markdown
---
name: <Agent display name>                   ← shown in TUI selector when mode: primary
description: <50-150 chars. User-facing selector blurb AND the
             auto-delegation matcher when mode: subagent.>
mode: primary | subagent                     ← from 2.10 dim 10
...
- mode: primary → "Switch to this agent when…" — user-facing, Tab-selected.
- mode: subagent → "The primary invokes me when…" — automatic delegation.>
...
When the user asks for "explicit invocation" on BOTH tools, the
canonical pairing is:

| Tool | Explicit-invocation mechanism |
|---|---|
| Claude | `slash-command.md` companion → `/<id>` types out in chat |
| Opencode | `mode: primary` → `<Display Name>` appears in Tab selector |
