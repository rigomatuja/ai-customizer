...
## 1.6 — Pre-write protocol (show is opt-in)

When the propose-draft-confirm cycle has produced a complete artifact
set and you are ready to write, you have TWO paths. The user picks.

### Step 1 — Ask once

Phrase it briefly when you reach the write-ready state:

> *"Ready to write. Show the full artifacts first, or go straight to
> writing?"*

The answer applies to ALL files in this batch (manifest + per-tool
bodies + companions). Re-ask only if a later batch in the same
conversation is genuinely independent (e.g., a second
`create` after the first finished).

If the user has ALREADY pre-declared a preference earlier in the
conversation, do NOT ask again. Recognised pre-declarations:

- **Skip show**: *"just write"*, *"no necesito ver"*, *"go ahead"*,
  *"directo"*, *"don't show me"*, *"escribe directamente"*.
- **Always show**: *"always show me"*, *"siempre muéstrame"*,
  *"never write without showing"*.

### Step 2A — User says "show"

Show every artifact in markdown code blocks BEFORE writing:

- `manifest.json` pretty-printed in a JSON fenced block.
- Each per-tool file (SKILL.md, agent markdown, patch before/after,
  optional slash-command companion) in its own fenced block with the
  target path as a heading.
- If many files, list them all — never hide any.

Then wait. The user approves (*"ok, write"* / *"go"*) or asks for
changes. On approval, write. On changes, iterate the field/section,
then re-show ONLY the changed parts (not the whole set again).

### Step 2B — User says "skip show"

Write all files atomically. After writing, announce the exact paths
that landed on disk. Briefly summarize what was written (one line
per file is enough). Remind the user of the next step.

### After writing — always

- Announce the absolute paths under `<catalogPath>/customizations/<type>/<id>/`.
- Remind: *"activate it from the UI's Catalog view and run Apply
  to install"* (or, for patches, *"go to Settings → Application
  Guide to register and run Apply"*).
- If a file write failed, see 3.11 Error handling.

### Rules

- **Asking is mandatory** (unless the user pre-declared) — never
  default silently to either path. The protocol is "ask once, then
  honor".
- **Never** write piecewise across show/skip transitions. The user's
  decision applies to the COMPLETE batch. If the user changes their
  mind mid-batch, finish the current path; ask again only on the
  next batch.
- **Never** skip the announcement of written paths, even on the
  skip-show path. The user must always know what landed where.
- **Never** treat absence of explicit answer as "skip" — re-ask
  briefly if the user gives an unrelated reply.
...
```
v<ver>/
├── claude/<id>.md                # installs to ~/.claude/agents/<id>.md globally, or <project>/.claude/agents/<id>.md for project scope
└── opencode/<id>.md              # installs to ~/.config/opencode/agents/<id>.md (plural per Opencode spec; `agent/` is backwards-compat only)
```
Filename matches `manifest.id` in both cases.

Claude project-scoped agents follow Claude Code's official project
subagent location: `<project>/.claude/agents/<id>.md`. Never put the
primary agent definition in `<project>/.claude/CLAUDE.md`; that file is
project memory/instructions. Use it only for a short reference when the
...
| **Subagent** | an `<id>.md` file in `~/.claude/agents/` or `~/.config/opencode/agents/` | Tool-dep | INVOKABLE — the primary delegates to it by description match, or the user invokes via Task/slash. Gentle-ai today installs few or none of these directly. |
...
- `ls ~/.config/opencode/agents/` minus `manager.md`.
...
- **Subagent** → `Read` the full agent body under
  `~/.claude/agents/` or `~/.config/opencode/agents/`. Same idea as
